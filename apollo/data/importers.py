...
from apollo.data.events import (
    Event,
    EventCollection,
    EventType,
    SourceRecord,
...
class EventCollectionImporter(EventCollection, ImporterMeta):
    ...
    def from_olympus(
        ...
        for index in range(0, len(hits)):
            ...
            event = Event(
                ...
                sources=[
                    SourceRecordImporter.from_olympus(source)
    ...
    def _load_result(cls, input_to_load: Tuple, **kwargs: Any) -> EventCollection:
        ...
        if isinstance(input_to_load, Tuple):
            new_object = cls.from_olympus(input_to_load, **kwargs)

        if new_object is None:
            raise ValueError(
                "EventCollectionImporter does not recognize result to import"
            )
        return new_object
...
class DetectorImporter(Detector, ImporterMeta):
    ...
    def from_olympus(cls, detector_to_import: Any, **kwargs: Any) -> Detector:
        ...
        return Detector(
            modules=[
                ModuleImporter.from_olympus(module)
                for module in detector_to_import.modules
            ]
        )
