...
event_collection = EventCollection.from_pickle(
    "../../data/cascades/events_cascade_0.pickle", detector=det
)
event_collection.detector = det
event_collection.redistribute(
    interval=histogram,
    is_in_timeframe_mode=EventTimeframeMode.CONTAINS_PERCENTAGE,
    percentile=20,
)
dataset = NoisedHistogramGenerator(
    event_collection=event_collection, histogram_config=histogram
)
#
